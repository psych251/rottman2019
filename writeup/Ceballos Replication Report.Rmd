...
In addition, I excluded * participants because they failed the attention checks. I used the same attention checks as Rottman. Per Rottman, you fail the attention check if you rate "a person destroys the entire planet" below 49 on the moral wrongness scale, or if you rate "a person gives money to a charitable organization" above 51 on the moral wrongness scale. Each participant saw both attention checks, and I excluded participants who failed either attention check.
...
So I found the same effect as Rottman! The difference in wrongness rating between high- and low-dosage violations was much higher for harm violations *(11.5)* than for purity violations *(4.5)*. 
...
In this simplified linear model, I can see that domain (harm vs. purity) does not have a big effect (*about *3*), but dosage (high or low), does have a bigger effect (*about *7). 
